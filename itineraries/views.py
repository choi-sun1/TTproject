...
from django.db.models import F  # 상단에 import 추가
...
class PlaceSearchAPIView(APIView):
    def get(self, request):
        try:
            ...
            # 장소 타입 필터링
            if place_type:
                conditions &= Q(place_type=place_type)
            
            places = Place.objects.filter(conditions).distinct()
            
            # 검색어와 정확히 일치하는 결과를 상위로
            exact_matches = places.filter(
                Q(name__iexact=query) |
                Q(region__iexact=query)
            )
            
            # 나머지 결과
            other_matches = places.exclude(
                id__in=exact_matches.values_list('id', flat=True)
            )
            
            # 정확한 매치와 나머지 결과 합치기
            places = list(exact_matches) + list(other_matches)
...
def wizard_step2(request):
    ...
    return render(request, 'itineraries/wizard/step2_places.html', context)
...
def wizard_step3(request):
    ...
    return render(request, 'itineraries/wizard/step3_schedule.html', context)
